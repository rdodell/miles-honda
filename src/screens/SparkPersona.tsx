import { useState } from 'react'
import { motion } from 'framer-motion'
import MilesMessage from '../components/MilesMessage'
import IanTyping from '../components/IanTyping'
import Tooltip from '../components/Tooltip'
import scenario from '../scenario.json'

interface SparkPersonaProps {
  onAdvance: (screen: string) => void
}

const s = scenario.screens['1.3']
const pc = s.personaCard

export default function SparkPersona({ onAdvance }: SparkPersonaProps) {
  const [showCard, setShowCard] = useState(false)
  const [showIanIdea, setShowIanIdea] = useState(false)
  const [showSupplement, setShowSupplement] = useState(false)
  const [showResearch, setShowResearch] = useState(false)

  return (
    <div className="flex flex-col gap-4 px-5 py-5 pb-20">

      {/* Beat 1 — Miles intro */}
      <MilesMessage text={s.milesIntro} onDone={() => setShowCard(true)} />

      {/* Laura persona card */}
      {showCard && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white border border-[#E8E4DE] rounded-2xl p-5 shadow-sm"
        >
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-full bg-[#CC0000] flex items-center justify-center text-white text-lg font-bold">L</div>
            <div>
              <div className="font-semibold text-base text-[#1A1A1A]">{pc.name} · {pc.age} · {pc.location}</div>
            </div>
          </div>
          <div className="space-y-3">
            <Section title="Day in the life" items={pc.dayInLife} />
            <Section title="Pain points" items={pc.painPoints} boldIndex={pc.painPointBoldIndex} accent />
            <Section title="What she'd pay for" items={pc.wouldPayFor} />
            <Section title="Where to find her" items={pc.whereToFind} />
          </div>
        </motion.div>
      )}

      {/* Beat 2 — Miles question */}
      {showCard && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <MilesMessage text={s.milesQuestion} onDone={() => setShowIanIdea(true)} />
        </motion.div>
      )}

      {/* Beat 3 — Ian's idea */}
      {showIanIdea && (
        <IanTyping message={s.ianIdea} onSent={() => setShowSupplement(true)} />
      )}

      {/* Beat 4 — Miles supplement */}
      {showSupplement && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <MilesMessage text={s.milesSupplement} onDone={() => setShowResearch(true)} />
        </motion.div>
      )}

      {/* Beat 5 — Research option cards */}
      {showResearch && (
        <motion.div
          initial={{ opacity: 0, y: 6 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex flex-col gap-3"
        >
          {s.researchOptions.map((option) => {
            if (option.primary) {
              return (
                <button
                  key={option.id}
                  onClick={() => onAdvance(option.advance!)}
                  className="text-left bg-[#CC0000] text-white rounded-2xl px-4 py-3.5 shadow-sm hover:bg-[#AA0000] transition-colors"
                >
                  <div className="font-semibold text-sm mb-1">{option.title}</div>
                  <div className="text-xs opacity-90 leading-snug">{option.description}</div>
                </button>
              )
            }
            return (
              <div key={option.id} className="w-full">
                <Tooltip text={option.tooltip!}>
                  <button className="w-full text-left bg-white border border-[#E8E4DE] rounded-2xl px-4 py-3.5 hover:bg-[#F2EEE8] transition-colors">
                    <div className="font-semibold text-sm text-[#1A1A1A] mb-1">{option.title}</div>
                    <div className="text-xs text-[#6B6570] leading-snug">{option.description}</div>
                  </button>
                </Tooltip>
              </div>
            )
          })}
        </motion.div>
      )}

      {/* After cards — Miles recommendation */}
      {showResearch && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <MilesMessage text={s.milesRecommendation} />
        </motion.div>
      )}

    </div>
  )
}

function Section({ title, items, boldIndex, accent = false }: { title: string; items: string[]; boldIndex?: number; accent?: boolean }) {
  return (
    <div>
      <div className="text-[11px] font-semibold uppercase tracking-wide text-[#A09A94] mb-1">{title}</div>
      <ul className="space-y-0.5">
        {items.map((item, i) => (
          <li key={i} className={`text-sm leading-snug ${accent && i === boldIndex ? 'font-semibold text-[#1A1A1A]' : 'text-[#6B6570]'}`}>
            · {item}
          </li>
        ))}
      </ul>
    </div>
  )
}
